 ...
 */
export class SpatialIndex<V, ID> extends Index<V, ID> {
    declare items: ID[][];
    declare cellSize: number;

    constructor(items: ID[][] = [], { cellSize = 5 } = {}) {
        super(items)
     ...
     */
    get(value: V): ID[] {
        const hash = this.hash(value as number[]);
     ...
     */
    has(value: V, id: ID | undefined): boolean {
        const hash = this.hash(value as number[]);
        this.items[hash] = this.items[hash] || [];
        const ids = this.items[hash];
        if (id === undefined) {
     ...
     */
    hash2d(x: number = 0, y: number = 0): number {
        const ix = Math.floor((x + 1000) / this.cellSize),
            iy = Math.floor((y + 1000) / this.cellSize);
     ...
     * @returns {number} The hash of the 3D value.
     */
    hash3d(x: number = 0, y: number = 0, z: number = 0): number {
        const ix = Math.floor((x + 1000) / this.cellSize),
            iy = Math.floor((y + 1000) / this.cellSize),
            iz = Math.floor((z + 1000) / this.cellSize);
    ...
    query(query: V): ID[] {
        ...
        const dims = (query as any[]).length === 2 ? 2 : 3
        if (dims === 2) {
            const [x, y] = query as number[];
